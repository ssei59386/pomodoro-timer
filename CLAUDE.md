...
2. The new "毎日の勉強できる時間はまだ入力されていません。" notice (`WeeklyScheduleEditor.tsx`) shows immediately on a fresh Onboarding form (before the user has touched anything), which can read as a premature warning — consider suppressing it when `showInitialSlots` is true, or rewording it as a neutral input hint there.
