...
- **Phase 6はon-device visual QA未実施**：このセッションの環境にはPlaywright/chromium-cli等のブラウザ自動化ツールが無く、dev server起動確認とjsdomテストのみ（直前セッションではPlaywrightが使えており環境依存の可能性が高い）。次回ツールが使えるなら Phase 6分（除外・スピルオーバーの見え方、Dashboardの見通しセクション）を確認するとよい。
