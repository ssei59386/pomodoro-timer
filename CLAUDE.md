...
**Scope**: Japanese 中学1〜3年 + 高校 math, chapters broken into fairly granular subtopics per the user's explicit request ("各章の小項目はそこそこ細かめにして調べてほしい"), each subtopic labeled with a difficulty estimate. **Default assumption, not yet confirmed with user**: 高校 scope = 数I・数A・数II・数B only (数III/C intentionally excluded by default, since Phase 0 targets 定期テスト for the general student population rather than advanced science-track/university-entrance prep — confirm with the user if this assumption is wrong before finalizing).
...
**Recommended execution plan for whoever picks this up**: dispatch several parallel research agents (e.g., one per grade/subject block: 中1, 中2, 中3, 数I, 数A, 数II, 数B), each cross-checking its chapter → subtopic breakdown via web search against real sources, each producing a structured chunk (chapter name → subtopic names → difficulty label, 1–3 scale to match the existing `ChapterMetadata.difficultyLevel` convention or a finer scale if useful), then compile all chunks into a single data file (suggested location: `src/data/mathCurriculumReference.ts`, exact shape TBD by whoever implements it).
