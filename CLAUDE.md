...
**Scope (confirmed)**: Japanese 中学1〜3年 + 高校数学 **including 数III・数C** (the earlier draft of this brief had excluded III/C by default — the user explicitly asked to include them, so full scope is 中1・中2・中3・数I・数A・数II・数B・数III・数C). Chapters broken into fairly granular subtopics per the user's explicit request ("各章の小項目はそこそこ細かめにして調べてほしい"), each subtopic labeled with a difficulty estimate.
