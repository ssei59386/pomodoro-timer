...
## In-progress feature: "見通し" (pace/progress forecast) — Phase 1 & 2 COMPLETE (2026-07-02), Phase 3 next

**Status: scope is fully decided (user override on record below); Phase 1 and Phase 2 are now fully implemented and tested.** See "Phase 1 current implementation state" and "Phase 2 current implementation state" at the bottom of this section for exactly what's done. Resume at Phase 3 (see Phase breakdown below).
...
Implemented directly (no subagent delegation) per this session's instructions:
...
**Next**: Phase 3 (UI wiring — Onboarding/Settings get basic/advanced problem count inputs + live curriculum-suggest dropdown; `SessionRecord` gets subtopic selection when a chapter has subtopics; also fixes the pre-existing bug where Onboarding's per-subtopic `selfReport` is computed into the chapter average and discarded instead of being persisted as `subtopic.understanding`). Entering/recording subtopic data still has zero visible effect in the app's actual plan generation until Phase 5, as designed — Phase 3 only adds the input UI.
