...
- **フェーズ7：バッファ＋フェーズ別の並び替え（「家庭教師の勘」の再現、2026-07-27追加）**：ユーザーが実際の家庭教師の勉強計画の立て方（可処分時間の7割程度で計画を組む／テスト直前は解けそうな項目を優先／未着手の項目は早めに触る）を再現したいという要望から、CEO/CTOレビューを経て追加。問題単位の完了管理（丸をつけて二度と出さない）は意図的にスコープ外（データモデルの作り直しが必要な大きな話のため）。
  - `plannableMinutesForDate(availability, date, subjects)`：`availableMinutesForDate`が返す可処分時間に、通常期は`BUFFER_RATIO`(0.7)、テストまで残り`FINAL_CRAM_DAYS_THRESHOLD`(3)日以内の「直前仕上げ期」は`FINAL_CRAM_BUFFER_RATIO`(0.85)を掛けた「計画に積んでよい時間」を返す。`generateTodayPlan`/`simulateForward`自体のシグネチャは変えず、呼び出し元（`store.tsx`の`ensureTodayPlan`、`simulateForward`内部）でこの関数に差し替えることで反映している。
...
**このセッションで2つの機能を実装し、両方ともcommit・push済み（本番反映）。** 最新commitは`caa8a1e`（1つ前が`619161d`）。テスト478件全通過・tscクリーン。詳細な経緯・決定事項は自動メモリの`project_ai_strategy_chat_2026_07_26.md`と`project_study_plan_buffer_phase_2026_07_27.md`に記録済み、要点のみここに残す。

- **① 「今日の作戦」AIチャット（commit `619161d`）**：既存のAI壁打ち相談（後悔防止トリガー限定）に、ダッシュボード発の2つ目の利用シーンを追加。ユーザーが「相談役でなくデータを踏まえた戦略提案がほしい」と要望→CEO/CTOレビューの結果、理解度の生数値は送らず`shortfallCount`/`onTrackCount`/`topPriorityLabel`という件数の要約ラベルのみを渡す設計に絞り、AIの役割は「どの章を切るべきか」を断定せず**問いかけに徹する**ことに限定した。詳細はCLAUDE.md本体の「AI advice」セクション参照（既に更新済み）。ux-reviewerが「未着手教科の不足が画面の他の場所に根拠なく表示される」重大バグを発見→修正済み。
- **② 計画バッファ＋フェーズ別並び替え（commit `caa8a1e`、フェーズ7）**：ユーザーが「実際の家庭教師の勉強計画の立て方」を再現したいと要望（可処分時間の7割で計画を組む／テスト直前は解けそうな項目を優先／未着手は早めに触る）。問題単位の完了管理（丸をつけて二度と出さない）は**ユーザー自身が明示的にスコープ外**と判断（データモデルの作り直しが必要な大きな話のため）。詳細はCLAUDE.md本体の「Core algorithms」節「フェーズ7」参照（既に追記済み）。
  - 実装後、Playwright実機確認で**4件の不整合**を発見・修正済み（誤った「🎉目標達成」表示、バッファ前の生の値のままだった「合計X分/Y分」の分母、新しい並び替えの理由が理由チップに出ない問題、後悔防止トリガーの発火判定がバッファで過敏になりすぎる問題）。特に最後の1件は実測（典型的な生徒データでshortfall判定の章が4件→6件に増加）で検証した上で、ユーザー判断によりトリガーだけバッファ無しの従来基準に切り離した（`simulateForward`の`budgetMode: "raw"`）。
- **教訓（次回同種の作業で踏襲すること）**：①「選定時の実際の割当」と「再描画時の再計算」が別ロジックである設計（`generateTodayPlan`の貪欲割当 vs `buildPlanFromItemKeys`の毎回再計算）は、新しい制約を追加するときに両者が食い違う典型的な罠になる。実装後は必ずPlaywrightで実機確認すること（今回もテストは全部通っていたのに実機で初めて発覚したバグが複数あった）。②新しい計算ロジックが既存の警告・通知系（後悔防止トリガー等）に影響する場合、単体テストだけでなく実測データで発火頻度への影響を確認してから統合すること。③CEO/CTOの意見が割れた時、無理にどちらかを選ばず両方の懸念を解消する第三案を探る価値がある（今回のフェーズ境界の決め方＝CTOの「日数でなくデータ駆動」案がその好例）。
- **未着手のまま持ち越し**：前々回セッションから続く「見通し機能の効能をHome画面でもっと日常的に感じてもらう」という方向性（`project_streak_backup_2026_07_26.md`参照）は今回も未着手。次に有力な検討テーマとして残っている。
