...
1. Sub-topics entered during onboarding (`averageInitialUnderstanding` path) cannot be edited or removed afterward — no UI for it post-onboarding.
2. ux-reviewer's final pass on this session's changes flagged (not blocking, but worth fixing next): Onboarding's top-level validation error (`error` state, `Onboarding.tsx` ~line 462) only renders near the submit button, so a mistake in an earlier section (test date, or now also "勉強できる時間を少なくとも1つ設定してください") isn't visible without scrolling down — consider `scrollIntoView` to the offending section, or converting to inline per-section errors (the pattern already used inside `WeeklyScheduleEditor`/`CalendarOverrides`/`DateOverridesList` for individual slot validation). Also: the new "毎日の勉強できる時間はまだ入力されていません。" notice shows immediately on a fresh Onboarding form (before the user has touched anything), which can read as a premature warning — consider suppressing it when `showInitialSlots` is true, or rewording it as a neutral input hint there.
3. Calendar cell tap targets in `CalendarOverrides.tsx` (Settings screen only) are likely under 44px.
4. No tests for `store.tsx`, `storage.ts`, or any component.
